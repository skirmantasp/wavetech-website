export default function Footer({ onOpenContact }: { onOpenContact: (subject: string) => void }) {
  return (
    <footer id="footer" style={{ background: 'var(--color-wt-black)' }}>
      ...
      <div style={{ padding: '8rem 4rem', borderBottom: '1px solid var(--color-wt-border)', textAlign: 'center', background: 'var(--color-wt-deep)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div style={{ position: 'relative', zIndex: 1 }}>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center' }}>
            <button className="btn-primary" onClick={() => onOpenContact('Deployment Request')}>
              Initialize Deployment
            </button>
            <button className="btn-outline" onClick={() => onOpenContact('Validation Data Request')}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)' }}>
        ...
        <div style={{ padding: '4rem', borderRight: '1px solid var(--color-wt-border)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            <a href="#" onClick={(e) => { e.preventDefault(); onOpenContact('Executive Briefing'); }} className="nav-link">Executive Briefing</a>
